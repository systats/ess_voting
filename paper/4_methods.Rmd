...
As part of this paper we built different statistical models with different repsonse functions. As detailed explained in section X and the dependent variable $y_i$ is labeled as 1 for voters of right wing populist parties estimated by model based-clustering. 
...
This binary data structure is best fitted by logistic regression and more generally by multilevel logistic regression. The latter takes into account that respondents are nested in their country context [@gelman07]. The formal model is defined as

$$Pr(y_i = 1) = logit^{-1}(\alpha_{j[i]} + \beta x_{[i]} + \gamma_jz_{[i]})$$

By deploying random effects with context level predictors we are able to model country-specific heterogenity and group level variance [@steenbergen02]. Additionally we avoid fixed effects for nested data to control model complexity and prevent overfitting. The random intercepts have the shape 

$$\alpha_{j} \sim N(\mu_\alpha, \sigma^2_{country})$$


*This means that the level-1 disturbances $\epsilon_{i}$ are drawn from a normal distribution with mean $0$ and variance $\sigma^2$ [@gelman07]. Since we expect justification rationality to be important for individual and group level, we include the groups mean as level 2 predictor (generally noted with $z_{q}$ $(q=1,... Q)$). Thus, the level-2 model for the random intercept model is:*

*According to Gelman and Hill [@gelman07], multi-level models appropriately estimate with a minimum of five level-2 units. Furthermore, Gelman and Hill address the impact of little group variation. If this is the case, the multi-level model is reduced to a classical regression without any group indicators. However, multi-level models control for the remaining intra-class or cluster correlation and do not perform poorer than standard regressions on the same data (steenbergen02; gelman07). Finally, we used the restricted maximum likelihood estimator (REML) which performs less biased under the conditions of little group variation as well as a small number of groups [@shikano14].*
